...
class Trainer():
    ...
    def train(self):
        ...
        t_first_make_batch = time.perf_counter()
        print(f"First make_batch: {t_first_make_batch - t_start:.6f}s")
        
        if self.data_iter is not None:
            d = next(self.data_iter)
        t_first_megadepth = time.perf_counter()
        print(f"First megadepth: {t_first_megadepth - t_first_make_batch:.6f}s")
        ...
        with tqdm.tqdm(total=self.steps) as pbar:
            for i in range(self.steps):
                ...
                if not self.dry_run:
                    if self.data_iter is not None:
                        try:
                            ...
                            self.data_iter = iter(self.data_loader)
                            d = next(self.data_iter)
                    t_megadepth = time.perf_counter()
                    if self.augmentor is not None:
                        #Grab synthetic data
                        p1s, p2s, H1, H2 = make_batch(self.augmentor, difficulty)
                    t_synthetic = time.perf_counter()

                if d is not None:
                    for k in d.keys():
                        if isinstance(d[k], torch.Tensor):
                            d[k] = d[k].to(self.dev)
                    # tensors_to_move = [k for k in d.keys() if isinstance(d[k], torch.Tensor)]
                    # for k in tensors_to_move:
                    #     d[k] = d[k].to(self.dev)
                
                    p1, p2 = d['image0'], d['image1']
                    positives_md_coarse = megadepth_warper.spvs_coarse(d, 8)
                t_md_transfer = time.perf_counter()
                # print(f"Transfer to device: {t_md_transfer - t_synthetic:.6f}s")
                ...
                # print(f"Prepare data: {t_prepare - t_start:.6f}s load_megadepth: {t_megadepth - t_start:.6f}s load_synthetic: {t_synthetic - t_megadepth:.6f}s")
                ...
                # print(f"Forward pass: {t_forward - t_prepare:.6f}s")
                ...
                # print(f"Loss computation: {t_loss - t_forward:.6f}s")
                ...
                # print(f"Backward pass: {t_end - t_loss:.6f}s")
                # print(f"Step {i+1}/{self.steps} took {t_end - t_start:.6f}s")
